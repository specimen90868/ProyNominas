...
namespace Nominas
{
    public partial class frmGenerarRecibos : Form
    {
        ...
        private void btnMostrar_Click(object sender, EventArgs e)
        {
            ...
            {
                cnx.Open();
                lstPeriodos = nh.obtenerPeriodosNomina(GLOBALES.IDEMPRESA, int.Parse(nudAnio.Value.ToString()), int.Parse(nudMes.Value.ToString()), 
                    tipoNomina, int.Parse(cmbPeriodo.SelectedValue.ToString()));
                cnx.Close();
                cnx.Dispose();
            }
            catch (Exception)
